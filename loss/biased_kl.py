...
class BiasedKL(nn.Module):
    ...
    def biased_divergence(self, pred, trg, biased_trg, biased_offset):
        B, S, V = pred.shape
        trg_ampl = (self.trg_factor - biased_offset).contiguous().view(-1)

        biased_dist = torch.zeros_like(pred)
        biased_dist = torch.scatter(biased_dist, 2, biased_trg.unsqueeze(-1), biased_offset.unsqueeze(-1))
        ...
        # prior (uniform)
        dist = self.ls * torch.ones_like(pred) / (V - 2)
